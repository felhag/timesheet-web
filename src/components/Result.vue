<script lang="ts">
import { ConfiguredTimesheet, HOME, Location } from '@/model/model';
...
export default {
  ...
  data() {
    return {
      home: undefined as Location | undefined
    }
  },
  beforeMount() {
    this.home = this.locations!!.find(loc => loc.name === HOME);
  },
  methods: {
    downloadTimesheet() {
    ...
    thuiswerkdagen() {
      return this.timesheet!.days.filter(day => day.location === this.home!.id).length;
    },
    kilometers() {
...
<template>
  ...
  <div>
    ...
    <div>
      Thuiswerkvergoeding {{ timesheet!.monthDisplay }} ({{ thuiswerkdagen() }} dagen * €2,40):
      {{ currency(thuiswerkdagen() * 2.4) }}
      <v-btn v-on:click="copy($event)" icon="mdi-content-copy"></v-btn>
    </div>
  </div>
</template>
